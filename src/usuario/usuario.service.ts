...
export class UsuarioService {
  ...
  async updateProfile(userId: number, dto: UpdateProfileDTO) {
    ...
    if (dto.nuevaContraseña)
      user.contraseña = await bcrypt.hash(dto.nuevaContraseña, 10);
  ...
  async registerEmployee(dto: RegisterEmployeeDto, userPayload: any) {
    ...
    if (dto.id_cliente) {
      cliente = await this.clientesRepo.findOne({
        where: { id_cliente: dto.id_cliente },
      });
      if (!cliente)
        throw new NotFoundException(
          `Cliente/Empresa con ID ${dto.id_cliente} no existe`,
        );
    }

    if (dto.id_sucursal) {
      sucursal = await this.sucursalRepo.findOne({
        where: { id_sucursal: dto.id_sucursal },
      });
      if (!sucursal)
        throw new NotFoundException(
          `Sucursal con ID ${dto.id_sucursal} no existe`,
        );
    ...
    if (role === 'CLIENTE_EMPRESA') {
      if (targetUser.id_cliente !== clienteId)
        throw new UnauthorizedException(
          'No puedes modificar roles de usuarios que pertenecen a otra empresa',
        );
      ...
      if (['ADMINISTRADOR', 'CLIENTE_EMPRESA'].includes(targetUser.rol.nombre))
        throw new UnauthorizedException(
          'No tienes permisos para alterar el rol de este usuario',
        );
    }
  ...
  async deactivateUser(targetUserId: number, userPayload: any) {
    ...
    if (targetUserId === requesterId) {
      throw new BadRequestException(
        'No puedes desactivar tu propia cuenta. Si deseas hacerlo, contacta a soporte.',
      );
    }
    ...
    if (role === 'CLIENTE_EMPRESA') {
      ...
      if (targetUser.id_cliente !== clienteId) {
        throw new UnauthorizedException(
          'No puedes desactivar usuarios que pertenecen a otra empresa',
        );
      }
      // No puede desactivar al Administrador ni a otros Gerentes (CLIENTE_EMPRESA)
      if (
        ['ADMINISTRADOR', 'CLIENTE_EMPRESA'].includes(targetUser.rol.nombre)
      ) {
        throw new UnauthorizedException(
          'No tienes permisos para desactivar cuentas con este nivel de acceso',
        );
      }
    }
  ...
  async activateUser(targetUserId: number, userPayload: any) {
    ...
    if (role === 'CLIENTE_EMPRESA') {
      if (targetUser.id_cliente !== clienteId) {
        throw new UnauthorizedException(
          'No puedes reactivar usuarios que pertenecen a otra empresa',
        );
      }
      if (
        ['ADMINISTRADOR', 'CLIENTE_EMPRESA'].includes(targetUser.rol.nombre)
        ...
        throw new UnauthorizedException(
          'No tienes permisos para reactivar cuentas con este nivel de acceso',
        );
      }
    }
  ...
  async reassignUser(userId: number, dto: ReassignUserDto, userPayload: any) {
    ...
    if (targetUser.rol.nombre === 'ADMINISTRADOR')
      throw new BadRequestException(
        'No se puede reasignar a un usuario con rol ADMINISTRADOR',
      );
    ...
    if (role === 'CLIENTE_EMPRESA') {
      ...
      if (targetUser.id_cliente !== clienteId)
        throw new UnauthorizedException(
          'No puedes reasignar usuarios que pertenecen a otra empresa',
        );

      //No puede transferir su personal a otras empresas
      if (dto.id_cliente && dto.id_cliente !== clienteId) {
        throw new BadRequestException(
          'No tienes permisos para transferir usuarios a otras empresas',
        );
      }
    ...
    if (dto.id_sucursal) {
      ...
      if (sucursal.id_cliente !== targetClienteId) {
        throw new BadRequestException(
          'La sucursal indicada no pertenece a la empresa de este usuario',
        );
      }
