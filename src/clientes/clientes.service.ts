...
export class ClientesService {
  ...
  async automaticDeactivation() {
    ...
    if (clientesVencidos.length > 0) {
      ...
      console.log(
        `Se encontraron ${clientesVencidos.length} clientes con planes vencidos`,
      );
    }
  }
  ...
  async notifyExpiringPlans() {
    ...
    if (clientesExpirando.length > 0) {
      for (const cliente of clientesExpirando) {
        ...
        const notificacion = {
          ...
          titulo:
            diffDays === 0
              ? 'Tu plan vence hoy'
              : `Tu plan vence en ${diffDays} día(s)`,
          Mensaje:
            diffDays === 0
      ...
      console.log(
        'No se encontraron clientes con planes que expiran en 7 dias',
      );
  }
  ...
  async create(dto: CreateClienteDto, suc: Partial<CreateSucursalDto>) {
    ...
    if (exists)
      throw new ConflictException(
        `Ya existe un cliente con ese documento ${dto.numero_documento}`,
      );

    const planIdAsignar = dto.id_plan ? dto.id_plan : 1; // Plan 1 por defecto
    const planExists = await this.planesRepo.findOne({
      where: { id_plan: planIdAsignar },
    });
    if (!planExists)
      throw new NotFoundException(
        `El plan con el ID ${planIdAsignar} no existe en el sistema`,
      );
    //Asignar fecha de finalizacion del plan, si no se proporciona se asigna 30 dias a partir de la fecha actual
    const fechaInicio = dto.fecha_inicio_plan
    ...
    if (dto.fecha_finalizacion_plan) {
      ...
      if (isNaN(fechaFinalizacion.getTime()))
        throw new BadRequestException(
          'La fecha de finalización enviada no es un formato válido.',
        );
    } else {
    ...
    const nuevoCliente = this.clientesRepo.create({
      ...
      costo_negociado:
        dto.costo_negociado !== undefined
          ? dto.costo_negociado
          : planExists.precio,
      limite_equipos_contratado:
        dto.limite_equipos_contratado !== undefined
          ? dto.limite_equipos_contratado
          : planExists.limite_equipos,
      is_active: true,
    });
    ...
    if (suc.id_cliente && suc.id_cliente !== clienteGuardado.id_cliente) {
      throw new BadRequestException(
        'El ID del cliente en el DTO de sucursal no coincide con el cliente creado',
      );
    }
  ...
  async update(id: number, dto: Partial<CreateClienteDto>) {
    ...
    if (
      ...
      dto.numero_documento !== cliente.numero_documento
    ) {
      const exists = await this.clientesRepo.findOne({
        where: { numero_documento: dto.numero_documento },
      });
  ...
  async deactivate(id: number) {
    ...
    return {
      message: `Cliente ${cliente.nombre_principal} y sus sucursales han sido desactivados`,
    };
  }
  ...
  async reactivate(id: number) {
    ...
    const cliente = await this.clientExists(id);
    if (cliente.is_active)
      throw new BadRequestException('El cliente ya se encuentra activo');
    ...
    return {
      message: `Cliente ${cliente.nombre_principal} y sus sucursales han sido reactivados`,
    };
  }
  ...
  async updatePlan(id: number, dto: UpdateContractDto) {
    ...
    if (!plan)
      throw new NotFoundException(
        `El plan con el ID ${dto.id_plan} no ha sido encontrado`,
      );
    ...
    if (dto.nuevoLimite !== undefined)
      cliente.limite_equipos_contratado = dto.nuevoLimite;
  ...
  private async clientExists(id: number): Promise<Clientes> {
    ...
    if (!cliente)
      throw new NotFoundException(
        `Cliente con el ID ${id} no ha sido encontrado`,
      );
    return cliente;
  }
}
