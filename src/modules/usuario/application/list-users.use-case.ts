...
export class ListUsersUseCase {
  ...
  private applyFilters(query: any, filters: GetUsersFilterDto): void {
    ...
    if (filters.id_usuario)
      query.andWhere('user.id_usuario = :idUsuario', {
        idUsuario: filters.id_usuario,
      });
    // Filtrado por nombre de rol
    if (filters.rolNombre)
      query.andWhere('rol.nombre = :rolNombre', {
    ...
    if (filters.cliente)
      query.andWhere('cliente.nombre_principal LIKE :cliente', {
        cliente: `%${filters.cliente}%`,
      });
    // Filtrado por nombre de sucursal
    if (filters.sucursal)
      query.andWhere('sucursal.nombre_sucursal LIKE :sucursal', {
